...
mb = MemoryBank(d_model=384, d_model_fine=192, use_dual_scale=True)
...
B, P, P2 = 2, 196, 784
feats_c = torch.randn(B, P, 384)
feats_f = torch.randn(B, P2, 192)
...
mb_single = MemoryBank(d_model=384, d_model_fine=192, use_dual_scale=False)
...
pa = PropagationAttention(d_model=384, d_key=256, d_value=256, n_heads=8)
query = torch.randn(B, P, 384)
with torch.no_grad():
    prop = pa(query, K2, V2)
assert prop.shape == (B, P, 384)
...
try:
    ...
    dec = SegmentationDecoder(dim_ssm=384, skip_dims=(384, 192, 96))
    assert dec.up1.proj.out_channels == 384
    assert dec.up2.proj.out_channels == 192
    assert dec.up3.proj.out_channels == 96
    print("[OK] SegDecoder channel dims")
    with torch.no_grad():
        out = dec(
            torch.randn(2, 3, 384, 196),
            {"stage_3": torch.randn(2,3,384,196), "stage_2": torch.randn(2,3,192,784), "stage_1": torch.randn(2,3,96,3136)}
        )
