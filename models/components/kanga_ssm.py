        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.use_checkpointing and x.requires_grad:
                ...
                x, n_state = cp.checkpoint(
                    lambda _x, _d, _ps, _id: layer(_x, _d, initial_state=_ps, return_last_state=True, identity=_id),
                    x, delta, p_state, identity, use_reentrant=False
                )
            else:
                res = layer(x, delta, initial_state=p_state, return_last_state=return_last_state, identity=identity)
