#!/bin/bash
#SBATCH --job-name=train_opt2
#SBATCH --partition=gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=24:00:00
#SBATCH --output=logs/slurm/train_opt2_%j.out
#SBATCH --error=logs/slurm/train_opt2_%j.err

# ==========================================
# Option 2 Training & Evaluation (DAVIS)
# Compressed Skips + High Capacity SSM + Add
# ==========================================

set -euo pipefail

echo "=========================================="
echo "  Option 2 Pipeline Started"
...
# 1. Training Phase
echo -e "\n[1/2] Training Option 2 (Compressed Skip Connections)..."
python train.py \
    datamodule=davis \
    ++trainer.max_epochs=100 \
    ++trainer.accumulate_grad_batches=2 \
    ++model.compress_skip=True \
    ++model.ssm_d_state=64 \
    ++model.ssm_layers=2 \
    ++model.identity_mode="add" \
    ++model.use_checkpointing=True \
    ++model.vos_loss_beta=0.2 \
    ++model.consistency_weight=0.1 \
    ++logger.name="opt2_compressed_skip_64" \
    "$@"
...
# 3. Evaluation Phase
echo -e "\n[2/2] Submitting DAVIS Evaluation Job (Opt2)..."
